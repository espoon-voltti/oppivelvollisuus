...
import fi.espoo.oppivelvollisuus.config.AUDIT_MARKER
 ...
 */
class AuditLogTests : FullApplicationTest() {
    @Autowired
    ...
    fun `createStudent emits CREATE_STUDENT audit event`() {
        controller.createStudent(testUser, minimalStudentAndCaseTestInput)
    ...
    fun `updateStudentCase emits UPDATE_STUDENT_CASE audit event with caseId`() {
        ...
        controller.updateStudentCase(testUser, studentId, caseId, minimalStudentCaseTestInput)
    ...
    fun `getEmployeeUsers emits GET_EMPLOYEES audit event`() {
        controller.getEmployeeUsers(testUser)
