...
class AsyncJobRunner<T : Any>(
    ...
    inline fun <reified P : T> registerHandler(noinline handler: (db: Database.Connection, clock: AppClock, msg: P) -> Unit) =
    ...
    fun enableAfterCommitHooks(clock: AppClock = RealAppClock()) =
        stateLock.write {
            afterCommitHooks =
                pools
                    .flatMap { pool ->
