...
data class FinishedInfo(
    ...
    val followUpMeasure: FollowUpMeasure?
) {
    init {
        ...
        if ((reason == CaseFinishedReason.COMPULSORY_EDUCATION_ENDED) != (followUpMeasure != null)) {
...
SELECT
    ...
    sc.follow_up_measure AS finished_info_follow_up_measure,
    sc.source,
    sc.source_valpas,
    sc.source_other,
    sc.source_contact,
...
"""
        ...
        .bind("followUpMeasure", data.finishedInfo?.followUpMeasure)
        .bind("user", user.id)
        .execute()
        .also { if (it != 1) throw NotFound() }
}
