...
import type { EspooUserResponse } from '../service-client.ts'
...
export class GatewayTester {
  ...
  private constructor(server: http.Server, sessionType: SessionType) {
    ...
    this.client.interceptors.request.use((config) => {
      if (this.setCsrfHeader) {
        config.headers.set('x-oppivelvollisuus-csrf', '1')
      }
      return config
    })
    ...
    this.nockScope = nock(oppivelvollisuusServiceUrl)
  }
  ...
  public async login(
    ...
    this.nockScope.post('/system/user-login').reply(200, user)
    await this.client.post(
      '/api/auth/saml/login/callback',
