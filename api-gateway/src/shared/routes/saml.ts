...
export interface SamlEndpointConfig<T extends SessionType> {
  ...
  providerUserCookieSecret?: string
}
...
export function createSamlIntegration<T extends SessionType>(
  ...
  const {
    ...
    defaultPageUrl,
    authenticate,
    providerUserCookieSecret
  } = endpointConfig

  const eventCode = (name: SamlAuditEvent) =>
    `oppivelvollisuus.saml.${strategyName}.${name}`
  ...
  if (providerUserCookieSecret) {
    router.use(cookieParser(providerUserCookieSecret))
  }
